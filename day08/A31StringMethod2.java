...
//두번째 String 메소드 연습 - 특정 문자열 안에서 찾기/치환, 문자열 추출 기능
public class A31StringMethod2 {
	

     public static void main(String[] args) {
		String email = "koreait.sec2020@gmail.com";    //*마지막 문자 m의 인덱스 = email.length()-1
		
		System.out.println("1. contains - 특정 단어(문자열)이 포함되어 있는지 검색");
		
		System.out.println("email.contains(\"email\") = " + email.contains("gmail"));     //"gmail"이 포함?
		System.out.println("email.contains(\"email!\") = "  + email.contains("gmail"));     //"gmail"이 포함?
		...
		//위의 2개는 리턴 타입이 boolean이므로 조건식으로 사용
	    ...
	    System.out.println(message.indexOf("kor", 7)); //15번 위치 문자부터 "kor" 찾아서 위치 리턴
		System.out.println(message.indexOf("kor", 16)); //15번 위치 문자부터 "kor" 찾아서 위치 리턴
		System.out.println(message.indexOf("kor", 30)); //15번 위치 문자부터 "kor" 찾아서 위치 리턴
		System.out.println(message.indexOf("kor", 37)); //15번 위치 문자부터 "kor" 찾아서 위치 리턴
		//찾는 문자열 "kor"이 정해진 범위의 문자열에서 없으면 -1 리턴
		
		System.out.println("4. replace = 문자열 치환");
		System.out.println("kor를 모두 KOREA로 치환하는 예시");
		...
		System.out.println("01234567890123456789012345678901234567890");
		System.out.println(message);
		System.out.println(message.substring(22)); //index 22부터 마지막까지
		System.out.println(message.substring(22, message.length())); //index 22부터 마지막까지
		System.out.println(message.substring(22, 35));        //index 22부터 35-22개 추출(34까지 35 포함 X)
