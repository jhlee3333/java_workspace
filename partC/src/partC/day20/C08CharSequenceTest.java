...
public class C08CharSequenceTest {
	//CharSequence(순서대로 나열된 문자) 인터페이스 : String, StringBuffer, StringBuilder가 구현 클래스
	public static void main(String[] args) {
	// 복습 : 해시코드는 객체의 주소로 만들어지는 객체의 식별을 위한 값입니다.
	//Integer.toHexString는 정수를 16진수로 변환하여 문자열 리턴. 16진수 구별을 "0x"를 앞에 붙입니다.
		...
		for(int i = 0; i < 5; i++) {
			...
			System.out.println(String.format("중간결과 result 객체 해시코드 down \n 10진수 = %d , 16진수 = 0x%8s\n" ,
		...
		System.out.println("+sb.delete(2, 4) = " + sb.insert(2, "***"));  //2번 인덱스부터 (4-2)개를 제거하기
		
		//StringBuilder는 StringBuffer와 같은 동작으로 구현된 클래스입니다.
		//단, StringBuffer는 멀티스레드에 안전합니다.
		
		System.out.println("\n3. String 클래스의 contains 메소드 인자는 CharSeqeunce 타입니다.");
