...
public class JavaVocaNote implements VocaNoteBook{
	...
	public JavaVocaNote() {
		//데이터를 1개씩 Map에 추가할 때 어떤 방식으로 저장할 것이냐?
//		voca = new HashMap<>();
...
//      voca = new LinkedHashMap<>();
	...
	public void load(String path) {		//파일에서 단어들 불러오기
		...
		try {
			...
			System.out.println("단어" + voca.size() + "개를 읽어와 저장했습니다.");
	...
	@Override
	public void save(String path) {        //Map에 있는 데이터를 파일로 저장하기
		File file = new File(path);
		PrintWriter pw = null;
		try {
			pw = new PrintWriter(file);
//			pw.println(voca);       //출력 테스트용
// Map 데이터 1개를 1줄로 출력하기 : Map 데이터 1개씩 가져오기는 iterator 또는 항상 for 사용
			...
			while(iterator.hasNext()) {
				...
				pw.println(vo);                      //JavaVoca의 toString 리턴 문자열 확인해보세요!!
			}
...
//		sc.close();              //표준입력 자원 해제 : main에서 표준 입력 못 합니다.
