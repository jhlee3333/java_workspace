...
-- 6) 기존 컬럼 삭제 : drop
-- 7) *기존 제약 조건 삭제 : drop constraint 제약조건 이름
...
-- 실행 후 : 컬럼 추가하기 전에 insert한 기존 행에도 기본값이 저장됩니다.
 ...
 * --> alter table ~ modify로 제약조건 설정은 컬럼 레벨. 주의해서 사용하기.
 */

-- tid의 길이를 20으로 변경하기
ALTER TABLE tbl_test# MODIFY (tid varchar2(20) NOT NULL);
-- 컬럼 레벨 제약조건 설정. constraint 제약조건 이름 not null에서 생략된 형태입니다.

-- 없던 제약조건 추가는 실행.
ALTER TABLE tbl_test# MODIFY (tname varchar2(25) UNIQUE);            -- 유일성(값 중복 안 됨)
...
-- 메타 데이터. 데이터 또는 db 객체들을 관리하는 데이터(데이터 사전)

ALTER TABLE "TBL_TEST#"
ADD                      -- 테이블 레벨로 제약조건 설정
CONSTRAINT test_pk       -- 제약조건 이름 생략 가능
...
-- 위 PK 추가하면 오류 : 중복된 값을 갖고 있으면 PK 추가 못합니다.

-- PK 없는 처음 상태에서 테스트하기
-- 이미 있는 not null 제약 조건도 잘 실행됩니다.(제약 조건 이름이 다르기 때문입니다.)
ALTER TABLE "tbl_test#"
ADD CONSTRAINT test_id              -- 개발자가 정하는 제약조건 이름
CHECK (tid IS NOT NULL);            -- NOU NULL 제약조건 추가는 CHECK 키워드로 합니다.
