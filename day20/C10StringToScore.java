...
public class C10StringToScore {
	...
	public static void main(String[] args) {
		...
		//만들어 보세요.
		System.out.println("Score 객체의 필드값 - name : 모모, java : 90, python : 88, html : 99");
		...
		while(stk.hasMoreTokens()) {
			temp[i++] = stk.nextToken();           //temp[i++] = stk.nextToken()은 배열에 대입 후 증가(++)
			//i++;
		...
		Score zuwi = new Score(stk.nextToken(),
				               ...
				               Integer.parseInt(stk.nextToken()),
				               Integer.parseInt(stk.nextToken()));
		System.out.println("변환된 Score 객체 => " + zuwi);
		System.out.println("zuwi 평균 =" + zuwi.average() + ", 학점 =" + zuwi.getGrade());
	}
