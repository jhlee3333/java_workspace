...
//		temp = "모모xy90xy88xy79xy33";     //1)
		...
		result = temp.split("xy");     //2)에서 정확히 문자열 "xy" 기준으로만 분리. 배열 크기
		System.out.println("temp 문자열 분리 split 결과 배열 : " + Arrays.toString(result));
		System.out.println(result.length);
		for(String t : result)
			System.out.println(t);
		
		//spilt 메소드는 분리된 결과를 배열로 짠~!!, 토크나이저는 nextToken()으로 직접 하나씩 가져오기
		System.out.println("\n2. 문자열 토크나이저 (토큰: 어휘분석의 단위를 가리키는 컴퓨터 용어이다.)");
		temp = "모모 90 88 79";
		
		StringTokenizer stk = new StringTokenizer(temp);      //구분기호 생략 : 공백과 줄 바꿈
...
//		System.out.println(stk.nextToken());    //오류 : next로 가져올 데이터 없음
		...
		System.out.println("\3. 기준문자열(delimiter)가 공백이 아닐 때");
//		temp = "모모xy90xy88xy79xy33";            //1)
		temp = "모모xy90xy88xy79xy33y999x1111";   //2)
		...
		while(stk.hasMoreTokens()) {
			...
			System.out.println("총 토큰 값 갯수 = " + count);   //예상 값은 5
		}
	}
