...
--    import와 export는 sql 명령이 있으나 디비버 메뉴로 하겠습니다.
--    [Tables] 항목 클릭 - 오른쪽 버튼 메뉴 [데이터 가져오기]
-- 3. 뷰, 인덱스
-- jdbc 후에 추가로 진도 내용 : 프로시저(함수), 트리거

SELECT count(*) FROM AAC_INTAKES ai ;                     -- 80187개
SELECT count(DISTINCT animal_id) FROM AAC_INTAKES ai ;    -- 72365개
-- animal_id가 중복된 값은 있어요. null은 없습니다.
SELECT count(animal_id) FROM AAC_INTAKES ai ;             -- 80817개

SELECT * FROM AAC_INTAKES ai
WHERE ANIMAL_ID = 'A740584';                              -- 실행 시간을 알아봅시다.
...
-- 테이블의 인덱스는 특정 컬럼값을 key, 컬럼값의 행이 저장된 주소는 value로 구성
-- 인덱스가 없다면 순차 검색
-- 검색 기능(=)을 향상시키기 위한 목적으로 만들어집니다. 컬럼값이 unique에 가까울수록
-- PK 컬럼은 자동으로 인덱스로 만들어집니다. 인덱스도 복합 컬럼으로 만들어질 수 있습니다.
CREATE INDEX animal_idx2
ON aaac_intakes(animal_type, color);

-- 인덱스 생성 전과 후 실행 시간 비교해보기
SELECT * FROM AAC_INTAKES ai
...
CREATE VIEW v_test           -- 조인 결과 SELECT로 가상(논리적) 테이블 생성
AS
SELECT
...
-- view를 대상으로 insert, update, delete할 수 있나요?
--   ㄴ   한 개의 물리적 테이블로 뷰를 만들었을 때만 가능.
