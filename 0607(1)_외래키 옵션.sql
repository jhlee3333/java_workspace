...
-- 외래키 옵션
-- 외래키로 참조관계가 설정되면 부모 테이블의 데이터가 삭제 시 오류
-- 옵션으로 다른 동작을 하도록 합니다.
...
-- 외래키의 on delete 옵션 3가지 테스트하기
-- 1. 아무것도 설정 안 하면 기본은 no action
ALTER TABLE TMP_SCORE
ADD
CONSTRAINT score_fk
FOREIGN KEY (stuno) REFERENCES tmp_student(stuno);
-- 참조하는 부모 테이블의 행을 삭제하기(조건식은 참조 컬럼 사용)
DELETE FROM TMP_STUDENT ts WHERE stuno = '2020017';        -- 오류 : 자식 테이블이 참조하고 있음
...
-- 2. cascade(계단식의) : 부모 테이블을 참조하고 있는 자식 테이블의 행을 삭제합니다.
ALTER TABLE TMP_SCORE
ADD
CONSTRAINT score_fk
FOREIGN KEY (stuno)
...
-- 데이터 삭제 테스트 후 다시 살리기 위해 트랜잭션 모드는 자동 커밋 해제
...
-- 3. set NULL
-- 외래키 컬럼이 기본키를 구성하는 컬럼 즉 식별 관계일 때는 동작 못 합니다.
-- 비식별관계에서 외래키 컬럼이 null 허용이 된 경우에만 가능합니다.

-- 참조관계를 비식별관계로 하기 위해 기본키 컬럼 추가(기본 기본키 삭제하기)
ALTER TABLE TMP_SCORE
