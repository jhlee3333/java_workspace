 ...
 * 조인 : 2개의 테이블 열(데이터 항목)을 합치는 것
   예시로 '구매 테이블의 구매 수량, 구매 날짜'와 '회원 테이블의 회원 이름, 이메일'을 공통 컬럼 동등 조건으로 합치기

   UNION 연산자 : 2개 테이블의 행(데이터 값)을 합치는 것
*
*/

-- 타입이 일치하는 2개의 컬럼을 선택해서 행 합치기. 타입 불일치하는 컬럼을 선택하면 오류.
SELECT
      NAME,
      EMAIL         -- 1) varchar2
...
UNION ALL
SELECT
      PCODE, PNAME        -- 2) 선택되는 컬럼 형식이 1)과 일치 - varchar2
FROM
      TBL_PRODUCT tp;
...
-- 컬럼의 갯수가 다른 경우
-- 2개 테이블 중 컬럼이 부족한 것을 null로 채워서 합치기
SELECT
      NAME, EMAIL, REG_DATE
FROM
      TBL_CUSTOM tc
...
-- 프로그래머스 문제 풀이 : 오프라인/온라인 판매 데이터 통합하기(lv.4)
