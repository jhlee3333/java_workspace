...
--     vage number(3,0);
       vname tbl_custom.name %TYPE;   --참조변수(타입 변수)
...
-- DBMS_OUTPUT는 콘솔에 출력하는 오라클 패키지의 하나이며 PUT_LINE 함수
    DBMS_OUTPUT.PUT_LINE('고객 이름 : ' || vname);      -- ||는 문자열 연결 연산
    DBMS_OUTPUT.PUT_LINE('고객 나이 : ' || vage);      -- ||는 문자열 연결 연산
    EXCEPTION           -- 예외(오류) 처리
    WHEN no_data_found THEN     -- no_data_found : 예외 이름
         DBMS_OUTPUT.PUT_LINE('찾는 데이터가 없습니다.');
END;

-- 인자와 리턴을 주는 형식. 저장 프로시저 정의하기
CREATE OR REPLACE PROCEDURE SERACH_CUSTOM(   --프로시저 이름 설정
...
BEGIN
	 ...
	 DBMS_OUTPUT.PUT_LINE('고객 이름 : ' || c_name);
     DBMS_OUTPUT.PUT_LINE('고객 나이 : ' || c_age);
	
END;

-- OUT 매개변수가 있는 프로시저 실행하는 형식
DECLARE
       vname tbl_custom.name %TYPE;
       vage tbl_custom.age %TYPE;
BEGIN
	   max_custom(vname, vage);          -- vname, vage는 OUT 매개변수 값을 받을 실제 변수입니다.
	   DBMS_OUTPUT.PUT_LINE('고객 이름 : ' || c_name);
       DBMS_OUTPUT.PUT_LINE('고객 나이 : ' || c_age);
END;
