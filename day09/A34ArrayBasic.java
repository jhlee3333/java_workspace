...
public class A34ArrayBasic {
	...
	//변수 선언은 저장할 데이터의 형식과 이름을 정의. 변수 선언 1개당 1개의 메모리 할당.
	
	//배열 선언은 저장할 데이터의 형식, 이름과 데이터의 갯수(할당받는 메모리 크기)
	int[] arr1 = {34, 56, 78, 89, 65};        //저장할 값 초기화와 크기 결정(4바이트X5 = 20바이트 할당)
	                                          //Index는 0~4
	
	int[] arr2 = new int[10];                 //초기값은 기본값 0, 정수 10개(40바이트 할당), Index 0~9
	
	//배열의 주요 속성 : length는 배열의 길이(갯수)를 갖습니다.
	...
	for(int i=0;i<arr1.length;i++) {
	    ...
	    for(int i1=0;i1<arr2.length;i1++) {
	    	arr2[i1] = (int)(Math.random()*45+1);
	    	System.out.println(arr2[i1] + "\t");
	    	if(i1==4) System.out.println();     //1줄에 5개 출력
	   
	    
	    	//Arrays 클래스에는 배열과 관련 static 메소드 있습니다.
	    	System.out.println("== Arrays 클래스 메소드 연습 ==");
	    	System.out.println("배열 요소의 값을 for문 없이 출력");
	    	System.out.println(Arrays.toString(arr1));
	    	System.out.println(Arrays.toString(arr2));
	    	System.out.println("배열 요소의 값을 순서대로 나열(정렬:sort)");
