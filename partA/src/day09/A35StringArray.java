...
//String 배열
//1) String 배열이 저장하는 값은 String 객체에 대한 참조값입니다.
//2) String 배열은 초기 문자열이 없고 크기만으로 선언했을 때 참조 상태는 null입니다.
public class A35StringArray {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<names.length;i++) {             //for(int i=0;~~~) i는 for의 블럭 지역변수
			System.out.println("이름 입력하세요.(종료:end) >>>");
		    names[i] = sc.nextLine();
		    //end 입력하면 for 중단.
		    if(names[i].equals("end")) break;       //참조타입은 == 등가 비교가 내용 비교 아닙니다.
		    
		    //사용자 입력을 받아 문자열 객체를 생성하고 names 배열이 문자열 객체를 참조
		    System.out.println(String.format("i=%d, keywords[%d] = %s(길이 : %d)\n",
